...
function genererSousOngletsComm() {
    ...
    if (stateComm.mainTab === 'auteurs' || stateComm.mainTab === 'oeuvres') {
        ...
        if (!['lexicaux', 'grammaticaux', 'figures', 'genre'].includes(stateComm.subTab)) stateComm.subTab = 'figures';
    }
...
function renderContentComm() {
    ...
    if (stateComm.mainTab === 'mouvements') {
        BDD.mouvements.forEach(mvt => {
            const autLinks = mvt.auteursPrincipaux.map(a => `<span class="font-bold text-blue-600">${a.nom}</span>`).join(', ');
            container.innerHTML += `
        ...
        BDD.auteurs.filter(a => a.genre === stateComm.subTab).forEach(aut => {
            const mvts = aut.mouvements.map(m => `<span class="text-xs font-bold px-3 py-1 bg-indigo-100 text-indigo-800 rounded-full">${m.nom}</span>`).join(' ');
            container.innerHTML += `
                <div class="p-8 bg-white border border-gray-200 rounded-2xl shadow-sm border-l-8 border-l-indigo-600">
        ...
        (BDD.procedes[stateComm.subTab] || []).forEach(cat => {
            ...
            cat.items.forEach(proc => {
                div.innerHTML += `
                    <div class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm flex flex-col md:flex-row gap-6">
